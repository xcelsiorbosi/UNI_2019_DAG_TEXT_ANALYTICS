...
def make_address(stations):
    for station in stations:
        address_rainfall = 'http://www.bom.gov.au/jsp/ncc/cdio/weatherData/av?p_nccObsCode=136&p_display_type=dailyDataFile&p_startYear=&p_c=&p_stn_num=' + station

        address_temp = 'http://www.bom.gov.au/jsp/ncc/cdio/weatherData/av?p_nccObsCode=122&p_display_type=dailyDataFile&p_startYear=&p_c=&p_stn_num=' + station
        address_wind = 'http://www.bom.gov.au/jsp/ncc/cdio/weatherData/av?p_nccObsCode=201&p_display_type=dwo&p_startYear=&p_c=&p_stn_num=' + station

        print(address_rainfall)
        print(address_temp)
        try:
            get_data(address_rainfall)
            get_data(address_temp)
        except:
            drop_list.append(station)
            continue


make_address(stations)
print(drop_list)

df = pd.DataFrame(data=drop_list, columns=['station'])
print(df)
stationID = 1
